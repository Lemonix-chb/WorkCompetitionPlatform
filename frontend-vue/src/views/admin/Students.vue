    ...
    <div class="container">
      ...
      <div class="page-header slide-in">
        <div class="header-content flex-between">
          <div>
            <h1 class="page-title">参赛者管理</h1>
            <p class="body-text">管理参赛学生信息和状态</p>
          </div>
      ...
      <div class="filters-section card card-flat scale-in mb-lg">
        <div class="filters-row flex gap-md">
          ...
          <button class="btn-secondary" @click="fetchStudents">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <!-- Students List -->
      <div class="students-section scale-in">
        <!-- Loading -->
        ...
        <!-- Data Table -->
        <div v-else-if="students.length > 0" class="data-table card">
            ...
            <el-table-column label="操作" width="280" fixed="right">
              <template #default="{ row }">
                <div class="action-buttons flex gap-sm">
                  ...
                  <button class="btn-action-edit" @click="editStudent(row)">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        ...
        <div v-else class="empty-state card">
          ...
          <h3 class="card-title mb-md">暂无参赛者数据</h3>
          <p class="caption">等待学生注册</p>
        </div>
      </div>
        ...
        <div v-if="selectedStudent" class="detail-content">
          <div class="detail-grid">
            <div class="detail-item">
              <span class="detail-label caption">学号</span>
              <span class="detail-value body-text">{{ selectedStudent.studentNo }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">姓名</span>
              <span class="detail-value body-text">{{ selectedStudent.realName }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">用户名</span>
              <span class="detail-value body-text">{{ selectedStudent.username }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">邮箱</span>
              <span class="detail-value body-text">{{ selectedStudent.email }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">联系电话</span>
              <span class="detail-value body-text">{{ selectedStudent.phone }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">专业</span>
              <span class="detail-value body-text">{{ selectedStudent.major }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">学院</span>
              <span class="detail-value body-text">{{ selectedStudent.college }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">状态</span>
              <span :class="getStatusClass(selectedStudent.status)" class="badge">
                {{ getStatusText(selectedStudent.status) }}
              </span>
            </div>
            <div class="detail-item">
              <span class="detail-label caption">注册时间</span>
              <span class="detail-value caption">{{ formatDate(selectedStudent.createTime) }}</span>
            </div>
          </div>
        </div>
      </el-dialog>
...
const approveStudent = async (student) => {
  try {
    await showConfirm(`确定要评审通过学生 "${student.realName}" 吗？`, '评审学生')
